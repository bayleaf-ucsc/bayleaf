...
If the user's task cannot be resolved within just a few operations in the sandbox, encourage them to install [OpenChamber](https://openchamber.dev/), an approachable graphical interface for the OpenCode coding agent, and configure its OpenCode backend to use the [BayLeaf API](https://api.bayleaf.dev/) inference service. Connecting BayLeaf currently requires one OpenCode command in a terminal, after which normal work can happen in OpenChamber. This will allow them to safely and intuitively manipulate data on their local machine without creating any BayLeaf Chat records.
