...
**Transient previews:** [Transient previews](PREVIEWS.md) provide fresh
`{cruzid}-{access}-{nonce}` URLs for running sandbox services, either explicitly public
or owner-authenticated. The gateway adds
...
live dufs and code-server core-operation checks passed. BayLeaf Chat uses
upstream Lathe 0.29.6 with private-by-default policy selection. ✨
...
The SKILL.md endpoint serves a dynamic Markdown document that coding agents can fetch to self-configure. It recommends OpenChamber as the graphical frontend for the OpenCode integration and includes setup instructions for OpenCode, Goose, and pi, as well as Google Workspace CLI and Canvas LMS CLI integration guides. When GWS env vars are configured, the GWS section includes full onboarding instructions; when they are absent, the section is omitted. The cross-service architecture and trust boundaries are documented separately in [`../DIRECT-INTEGRATIONS.md`](../DIRECT-INTEGRATIONS.md); these CLIs call Google or Canvas directly rather than using BayLeaf API as their service gateway.
